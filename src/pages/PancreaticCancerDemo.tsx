...
const PancreaticCancerDemo = () => {
  ...
  const [uploadedImages, setUploadedImages] = useState<{id: number, name: string, thumbnail: string, fullImage: string, hasAbnormality: boolean}[]>([]);
                    ...
                    <DialogTrigger asChild>
                      <div className="relative border-2 border-dashed rounded-lg overflow-hidden cursor-pointer transition-all hover:shadow-md border-muted-foreground/20 hover:border-primary/50">
                        <div className="aspect-square flex flex-col items-center justify-center gap-2 bg-muted/30">
                          ...
                          <span className="text-xs text-muted-foreground">Upload CT Scan</span>
                        </div>
                      </div>
                    </DialogTrigger>
                    <DialogContent>
              ...
              <div className="border-2 border-dashed border-muted-foreground/20 rounded-lg h-[400px] flex items-center justify-center overflow-hidden bg-black/5">
                {segmentationResult ? (
                  <div className="relative w-full h-full">
                    ...
                    {[...SAMPLE_SCANS, ...uploadedImages].find(s => s.id === selectedScan)?.hasAbnormality && (
                      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 border-2 border-red-500 rounded-full animate-pulse" />
                    )}
                  </div>
                ) : (
              ...
              {segmentationResult && (
                <div className="mt-4 bg-secondary/30 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    {[...SAMPLE_SCANS, ...uploadedImages].find(s => s.id === selectedScan)?.hasAbnormality 
                      ? "The model has detected regions of interest that might indicate pancreatic abnormalities. The highlighted area shows potential tissue changes that may require further investigation."
                      : "No significant abnormalities detected in the pancreatic region. The model analyzed the tissue density and structure patterns typical in healthy pancreas tissue."
                    }
                  </p>
